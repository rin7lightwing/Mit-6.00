...
class Hand(object):
    ...
    def update(self, word):
        ...
        """
        oldDict = self.handDict
        for c in word:
            oldDict[c] -= 1
            if oldDict[c] == 0:
                del oldDict[c]
        self.handDict = oldDict
        # Done
    def containsLetters(self, letters):
        ...
        False otherwise
        """
        freqLetter = getFrequencyDict(letters)
        for c in letters:
            if self.handDict.get(c,0) < freqLetter[c]:
                return False
        return True
        # DONE
    def isEmpty(self):
    ...
    def __str__(self):
        ...
        return string
#
# testHand = Hand(3, {'a':1, 'd':1})
# otherHand = Hand(3, {'a':1, 'd':1})
# print testHand
# print other
# print testHand == other
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        for word in wordlist.getList():
            if h.containsLetters(word) and getWordScore(word) > bestword_Value:
                bestword = word
                bestword_Value = getWordScore(word)
        return bestword
        # DONE
